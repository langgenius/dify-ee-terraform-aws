locals {
  ...
  opensearch_subnets = (
    ...
    length(var.opensearch_subnets) > 0 ? var.opensearch_subnets :
...
resource "aws_security_group" "opensearch" {
  ...
  tags = {
    Name        = "dify-${var.deployment_id}-opensearch-sg"
...
resource "aws_opensearch_domain" "main" {
  domain_name    = "dify-${var.deployment_id}-opensearch"
    ...
    Statement = [
      {
        ...
        Action   = "es:*"
        Resource = "arn:${local.aws_partition}:es:${var.aws_region}:${var.aws_account_id}:domain/dify-${var.deployment_id}-opensearch/*"
      }
    ]
  })

  tags = {
    Name        = "dify-${var.deployment_id}-opensearch"
    Environment = var.environment
  }
}
